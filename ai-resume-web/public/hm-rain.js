/* hm unified matrix rain for ndtool.cn web app.
   Self-styles its canvas; attach to any page:
     <canvas id="hm-rain"></canvas> + <script src="/hm-rain.js"></script>
   Respects prefers-reduced-motion. */
(function () {
  var cv = document.getElementById('hm-rain');
  if (!cv) return;
  cv.style.cssText = 'position:fixed;inset:0;z-index:0;opacity:.35;pointer-events:none';
  var media = window.matchMedia('(prefers-reduced-motion: reduce)');
  if (media.matches) { cv.style.display = 'none'; return; }
