...
export default function ResourcesListPage() {
  ...
  return (
    <PublicLayout>
      <ResourceLayout>
        <h1 className="text-3xl font-bold text-[#E8E4DE] mb-8">{t('resources.list.title')}</h1>

        <div className="space-y-10">
          {RESOURCE_SECTIONS.map((section) => (
            <section key={section.key}>
              <div className="flex items-center gap-3 mb-4 pb-2 border-b border-[#252525]">
                <div className="text-[#C84B31]">{section.icon}</div>
                <h2 className="text-lg font-semibold text-[#E8E4DE]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {section.items.map((item) => (
                  <div
                    key={item}
                    className="p-4 bg-[#161616] border border-[#252525] rounded hover:border-[#5A5652] transition-all cursor-default"
                  >
                    <h3 className="text-[#E8E4DE] text-sm font-medium">
