...
export default function FeaturesSection() {
  const ref = useSectionReveal<HTMLElement>({
    children: '.lp-feature-card',
    childFrom: { y: 64, autoAlpha: 0, scale: 0.97 },
    childStagger: 0.09,
  });
    ...
    (_context, contextSafe) => {
      ...
      mm.add(
        ...
        (ctx) => {
          ...
          if (!ok || !contextSafe) return;

          const cards = gsap.utils.toArray<HTMLElement>('.lp-feature-card', scopeRoot);
          if (!fine) {
            /* 触屏:入场时图标逐个落印 */
            cards.forEach((card) => {
              const icon = card.querySelector('.lp-feature-icon');
              if (icon) gsap.from(icon, { scale: 0, rotation: -18, duration: 0.6, ease: 'settle', scrollTrigger: { trigger: card, start: 'top 85%', once: true } });
            });
            return undefined;
          }
          ...
          cards.forEach((card) => {
            const icon = card.querySelector('.lp-feature-icon');
            if (icon) {
              gsap.from(icon, { scale: 0, rotation: -18, duration: 0.6, ease: 'settle', scrollTrigger: { trigger: card, start: 'top 85%', once: true } });
            }
            ...
            const onEnter = contextSafe(() => {
              if (icon) {
                gsap.to(icon, { scale: 1.18, rotation: 4, duration: 0.4, ease: 'settle', overwrite: 'auto' });
              }
            });
            const onLeave = contextSafe(() => {
              if (icon) {
                gsap.to(icon, { scale: 1, rotation: 0, duration: 0.45, ease: 'silk', overwrite: 'auto' });
              }
            });
  ...
  return (
    <section className="lp-features" ref={ref}>
