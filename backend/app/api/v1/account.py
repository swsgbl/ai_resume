...
@router.get("/bindings")
async def get_account_bindings(current_user: User = Depends(get_current_user)):
    """获取所有已绑定的账号"""
    bindings = {
        "email": {
...
async def bind_email(
    ...
    if result.scalar_one_or_none():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="该邮箱已被其他账号使用"
        )
...
async def bind_phone(
    ...
    if result.scalar_one_or_none():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="该手机号已被其他账号使用"
        )
...
async def unbind_account(
    ...
    if bound_count <= 1:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="至少需要保留一种登录方式"
        )

    # 解绑对应账号
    if request.account_type == "email":
        if not current_user.email:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="邮箱未绑定")
        current_user.email = None
        current_user.is_verified = False

    elif request.account_type == "phone":
        if not current_user.phone:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="手机号未绑定")
        current_user.phone = None

    elif request.account_type == "wechat":
        if not current_user.wechat_openid:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="微信未绑定")
        ...
        if not current_user.google_id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Google未绑定")
        ...
        if not current_user.github_id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="GitHub未绑定")
        ...
        if not current_user.gitee_id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Gitee未绑定")
        ...
        if not current_user.discord_id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Discord未绑定")
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="不支持的账号类型")
