...
async def register(user_data: UserCreate, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return Response(
        data={"user": UserResponse.model_validate(user), **token_response.model_dump()},
        message="注册成功",
    )
...
async def login_json(login_data: UserLogin, request: Request, db: AsyncSession = Depends(get_db)):
    """用户登录 - JSON格式"""
...
async def refresh_token(token_data: TokenRefresh, db: AsyncSession = Depends(get_db)):
    ...
    return Response(data=_create_token_response(user), message="令牌刷新成功")


@router.get("/me", response_model=None)
async def get_current_user_info(current_user: User = Depends(get_current_user)):
    """获取当前用户信息"""
    return Response(data=UserResponse.model_validate(current_user), message="获取成功")
...
async def send_sms_code(
    ...
    if not sms_service.rate_limiter.can_send_ip(client_ip):
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="IP请求过于频繁，请稍后重试"
        )
...
async def sms_login(request: Request, data: SMSLoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="该手机号未注册，请先注册"
        )
...
async def sms_register(
    request: Request, data: SMSRegisterRequest, db: AsyncSession = Depends(get_db)
):
    """手机号+验证码注册"""
    # 校验验证码
    if not await sms_service.verify_code(data.phone, data.code, data.sms_token):
    ...
    if result.scalar_one_or_none():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="该手机号已注册，请直接登录"
        )
