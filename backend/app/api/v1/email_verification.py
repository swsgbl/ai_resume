...
async def send_verification_code(request: Request, body: SendCodeRequest):
    ...
    try:
        ...
        success = await email_service.send_verification_email(email=body.email, code=code)
...
async def verify_code(
    ...
    try:
        ...
        return {"code": 200, "message": "验证成功", "data": {"verified": True}}

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"验证失败: {str(e)}"
        )
