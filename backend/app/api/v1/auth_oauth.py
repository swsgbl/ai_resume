...
from app.services.oauth_service import (
    get_google_provider, get_github_provider, get_gitee_provider,
    get_discord_provider, get_state_manager, oauth_login
)
...
async def gitee_authorize(redirect_uri: Optional[str] = None):
    ...
    return Response(data={"auth_url": auth_url, "state": state, "provider": "gitee"}, message="获取授权URL成功")


@router.post("/gitee/callback")
async def gitee_callback(request_data: OAuthLoginRequest, db: AsyncSession = Depends(get_db)):
    """处理 Gitee OAuth 回调"""
    provider_info = await oauth_login("gitee", request_data.code, request_data.state, request_data.redirect_uri)
...
async def discord_authorize(redirect_uri: Optional[str] = None):
    ...
    return Response(data={"auth_url": auth_url, "state": state, "provider": "discord"}, message="获取授权URL成功")


@router.post("/discord/callback")
async def discord_callback(request_data: OAuthLoginRequest, db: AsyncSession = Depends(get_db)):
    """处理 Discord OAuth 回调"""
    provider_info = await oauth_login("discord", request_data.code, request_data.state, request_data.redirect_uri)
...
async def unified_authorize(provider: str, redirect_uri: Optional[str] = None):
    ...
    return Response(data={"auth_url": auth_url, "state": state, "provider": provider}, message="获取授权URL成功")
