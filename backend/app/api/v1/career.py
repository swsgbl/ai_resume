...
async def evaluate_jd(
    ...
    resume_content = (
        json.dumps(resume.content, ensure_ascii=False)
        if isinstance(resume.content, dict)
        else str(resume.content)
    )
    ...
    try:
        ...
        ai_response = await ai_service.generate_content(
            system_prompt=prompt["system"], user_prompt=prompt["user"], temperature=0.7
        )

        parsed = _parse_ai_json(ai_response)

        return Response(data=parsed, message="评估完成")
...
async def generate_story_bank(
    ...
    try:
        ...
        ai_service = get_ai_provider()
        ai_response = await ai_service.generate_content(
            system_prompt=prompt["system"], user_prompt=prompt["user"], temperature=0.8
        )

        parsed = _parse_ai_json(ai_response)

        return Response(data=parsed, message="故事挖掘完成")
    except Exception as e:
        logger.error(f"故事银行生成失败: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"AI 服务暂时不可用: {str(e)}"
        )
...
{{
  ...
  "confidence_notes": "哪些修改是确信的，哪些需要用户确认"
    ...
    try:
        ...
        return Response(data=parsed, message="智能定制完成")
    except Exception as e:
...
def _parse_ai_json(response: str) -> dict:
    ...
    try:
        ...
        return {"raw_response": text[:1000], "error": "AI 返回格式异常，请重试"}
